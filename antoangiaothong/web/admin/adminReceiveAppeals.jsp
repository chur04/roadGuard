<%-- 
    Document   : adminReceiveAppeals
    Created on : 15 thg 3, 2025, 23:03:43
    Author     : im_chur
--%>
...
<head>
    ...
    <title>Quản Lý Khiếu Nại - Cyberpunk Admin</title>
        ...
        <div class="main-content">
            ...
            <div class="content-area">
                <div class="page-header">
                    <h1>Danh sách khiếu nại đang chờ xử lý</h1>
                    <p>Xem và xử lý các khiếu nại từ người vi phạm.</p>
                </div>
                
                <div class="filter-bar">
                    <div class="search-input">
                        <i class="fas fa-search"></i>
                        <input type="text" id="searchInput" placeholder="Tìm kiếm khiếu nại...">
                    </div>
                ...
                <c:choose>
                    <c:when test="${not empty pendingAppeals}">
                        <div class="data-table-container">
                            <table class="data-table">
                                ...
                                <tbody>
                                    <c:forEach var="appeal" items="${pendingAppeals}">
                                        <tr>
                                            <td>${appeal.appealID}</td>
                                            <td>${appeal.violationID}</td>
                                            <td>${appeal.violatorID}</td>
                                            <td class="truncate">${appeal.appealText}</td>
                                            <td>${appeal.appealDate}</td>
                                            <td>
                                                <c:choose>
                                                    <c:when test="${not empty appeal.imageURL}">
                                                        <a href="uploads/${appeal.imageURL}" target="_blank" class="media-link">
                                            ...
                                            <td>
                                                <c:choose>
                                                    <c:when test="${not empty appeal.videoURL}">
                                                        <a href="uploads/${appeal.videoURL}" target="_blank" class="media-link">
                                            ...
                                            <td>
                                                <div class="action-buttons">
                                                    <a href="${pageContext.request.contextPath}/AdminActionAppeal?action=view&id=${appeal.appealID}&violationID=${appeal.violationID}" class="btn btn-view">
                                                        <i class="fas fa-eye"></i> Xem vi phạm
                                                    </a>
                                                    <a href="${pageContext.request.contextPath}/AdminActionAppeal?action=response&id=${appeal.appealID}&violationID=${appeal.violationID}" class="btn btn-response">
                    ...
                    <c:otherwise>
                        <div class="empty-state">
                            ...
                            <h3>Không có khiếu nại nào đang chờ xử lý</h3>
                            <p>Tất cả các khiếu nại đã được xử lý hoặc chưa có khiếu nại nào được gửi.</p>
                        </div>
                    </c:otherwise>
                </c:choose>
            ...
            if (statusFilter) {
                statusFilter.addEventListener('change', function() {
                    ...
                    tableRows.forEach(row => {
                        ...
                        const text = row.textContent.toLowerCase();
                        const shouldShow = text.includes(selectedStatus);
